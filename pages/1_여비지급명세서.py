...
st.set_page_config(page_title="여비지급명세서 생성기", layout="wide")
show_sidebar()
...
def write_sheet(workbook, df, title, is_empty=False):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        worksheet.write_number(current_row, 10, ilbi, format_money)
        worksheet.write_number(current_row, 11, sikbi, format_money)
        worksheet.write_number(current_row, 12, 0, format_money)
        worksheet.write_number(current_row, 13, 0, format_money)
        worksheet.write_number(current_row, 14, 0, format_money)
    ...
    worksheet.write(total_row, 9, "", format_total_label)
