...
def amount_to_korean(num):
    ...
    part = num % 10000
    if part > 0:
        res = to_korean_chunk(part)
    num //= 10000
    if num > 0:
        part = num % 10000
        if part > 0:
            res = to_korean_chunk(part) + "만" + res
    num //= 10000
    if num > 0:
        part = num % 10000
        if part > 0:
            res = to_korean_chunk(part) + "억" + res
    return res
...
def _parse_excel_cached(file_bytes: bytes, label: str):
    ...
    data = data[
        data["성명"].notna()
        & (data["성명"].astype(str).str.strip() != "")
        & (data["성명"].astype(str) != "nan")
    ].copy()
    data["고용형태"] = label
    return data, None
...
def show():
    ...
    st.markdown(
        "새올에서 다운로드한 **[초과근무목록 액셀 파일]**을 올리면, "
        "내부결재용 공문 텍스트와 지출증빙 엑셀(2p, 3p)을 자동으로 생성합니다."
    )
    ...
    with col_up1:
        file_gongmuwon = st.file_uploader(
            "🧑‍💼 공무원 초과근무목록", type=['xls', 'xlsx'], key="upload_gongmuwon"
        )
    with col_up2:
        file_gongmujik = st.file_uploader(
            "👷 공무직 초과근무목록", type=['xls', 'xlsx'], key="upload_gongmujik"
        )
    ...
    try:
        ...
        parts = []
        if cnt_gm:
            parts.append(f"공무원 {cnt_gm}건")
        if cnt_gj:
            parts.append(f"공무직 {cnt_gj}건")
        st.success(f"✅ 데이터 인식 완료! 총 {len(data_df)}건 ({' + '.join(parts)})")

        # ══════════════════════════════════════════════════════════════
        # 2단계: 과 선택 → 이름 목록 표시 → 멀티셀렉트
        # ══════════════════════════════════════════════════════════════
        st.markdown("---")
        st.subheader("2. 대상자 선택")

        # 엑셀 부서 컬럼에서 과 목록 추출
        excel_divs = sorted({
            str(x).strip()
            for x in data_df["부서"].dropna()
        ...
        # 선택한 과에 해당하는 인원 목록 (중복 제거, 순서 유지)
        div_df = data_df[data_df["부서"].astype(str).str.strip() == selected_div]
        all_names_in_div = list(dict.fromkeys(
            str(x).strip()
            for x in div_df["성명"].dropna()
            if str(x).strip() not in ["", "nan"]
        ))

        if not all_names_in_div:
            st.warning(f"{selected_div}에 해당하는 인원이 없습니다.")
            return

        # 직급 정보 추출 (있으면)
        rank_col = next(
            (c for c in data_df.columns if "직급" in str(c) or "직위" in str(c)), None
        )
        rank_map = {}
        if rank_col:
        ...
        # ══════════════════════════════════════════════════════════════
        st.markdown("---")
        st.markdown("##### 🔢 인원 순서 지정")
        st.caption("첫 번째 = 확인자(팀장), 마지막 = 작성자(막내). 위/아래 버튼으로 순서를 바꿀 수 있습니다.")

        # session_state에 순서 관리
        order_key = f"name_order_{selected_div}"
        if order_key not in st.session_state or set(st.session_state[order_key]) != set(selected_names):
            # 새 선택이면 초기화 (기존 순서 유지하되, 빠진 사람 제거 + 새 사람 뒤에 추가)
            prev = st.session_state.get(order_key, [])
            kept = [n for n in prev if n in selected_names]
            new_adds = [n for n in selected_names if n not in kept]
            st.session_state[order_key] = kept + new_adds
        ...
        selected_names = ordered  # 정렬된 순서로 갱신
        ...
        filtered_data_df = filtered_data_df[
            pd.to_numeric(filtered_data_df["수당시간(분)"], errors='coerce').fillna(0) >= min_hours
        ]
        ...
        # 순서 기반 자동완성: 첫번째=확인자, 마지막=작성자
        default_confirmer = ""
        default_writer = ""
        if selected_names:
            leader = selected_names[0]
            default_confirmer = f"{rank_map.get(leader, '지방농업주사')} {leader}"
            writer = selected_names[-1]
            default_writer = f"{rank_map.get(writer, '지방농업서기')} {writer}"

        col1, col2, col3, col4 = st.columns(4)
        with col1:
            team_name = st.text_input("팀명", value="", placeholder="예: 스마트팜지원팀")
        with col2:
            target_month = st.text_input("기준 월", value=str(datetime.now().month))
        with col3:
            confirm_person = st.text_input(
                "확인자 (직급 이름)", value=default_confirmer, help="예: 지방농업주사 홍길동"
            )
        with col4:
            write_person = st.text_input(
                "작성자 (직급 이름)", value=default_writer, help="예: 지방농업서기 신사임당"
            )
        ...
        st.warning(
            f"아래 표에 선택하신 **{total_cases}건**의 내역이 나타났습니다. / "
            f" 💰 **이번 달 총 급식비 예상액: {total_amount:,}원** (단가 9,000원 기준) / "
            f"  **'급식장소'** 열을 더블클릭하여 직접 입력해주세요."
        )

        disp_df = filtered_data_df[
            ['근무일자', '부서', '고용형태', '성명', '휴일구분',
        ...
        disp_df = (
            disp_df
            .sort_values(by=['_name_order', '근무일자'])
            .drop(columns=['_name_order'])
        ...
        # 테이블 리셋 조건
        current_view_key = f"{selected_div}_{','.join(str(n) for n in selected_names)}_{min_hours}"
        ...
        unique_rests = [
            r for r in edited_df["급식장소"].unique()
        ...
        if len(unique_rests) > 0:
            ...
            _empty_cnt = int(
                edited_df["급식장소"].astype(str).str.strip().isin(["", "nan", "None"]).sum()
            )
            if _empty_cnt > 0:
...
붙임 급식비 집행내역 1부. 끝."""
            ...
            fmt_normal_border = workbook.add_format({
                'align': 'left', 'valign': 'vcenter', 'border': 1
            })
            fmt_right = workbook.add_format({'align': 'right'})
            fmt_sign = workbook.add_format({
            ...
            ws1.merge_range(
                "A4:C4",
                f"급식기간 : {cur_year}. {target_month}. 1. ~ {target_month}. {last_day}.",
                fmt_normal,
            )
            ...
            ws2.set_column(3, 3, 7)
            ws2.set_column(4, 4, 6)
            ws2.set_column(5, 5, 8)
            ws2.set_column(6, 6, 8)
            ws2.set_column(7, 7, 8)
            ws2.set_column(8, 8, 28)
            ws2.set_column(9, 9, 15)
            ws2.set_column(10, 10, 10)

            ws2.merge_range(
                "A1:J2",
                f"{team_name} 업무추진 급식비 세부집행내역 "
                f"[{target_month}. 1. ~ {target_month}. {last_day}.]",
                fmt_title,
            )
            ...
            ws2.merge_range(
                "H4:J4",
                f"새올초과근무내역 확인필 : {space_name(name_only)} (인)",
                fmt_right,
            )
            ...
            fmt_box_gray = workbook.add_format({
            ...
            headers = [
                "번호", "근무일자", "근무자", "고용형태", "구분",
                "출근", "퇴근", "수당시간", "근무내역", "급식장소", "급식비(원)"
            ]
            for i, h in enumerate(headers):
                ws2.write(7, i, h, fmt_header)

            fmt_gongmuwon = workbook.add_format({
                'align': 'center', 'valign': 'vcenter', 'border': 1, 'bg_color': '#EFF6FF'
            })
            fmt_gongmujik = workbook.add_format({
                'align': 'center', 'valign': 'vcenter', 'border': 1, 'bg_color': '#F0FDF4'
            })
